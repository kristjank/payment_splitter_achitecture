...
import {IPlatformToken} from "./interfaces/IPlatformToken.sol";

/**
 * @title PlatformTokenDistributor
 * @notice It handles the distribution of Platform token.
 * It auto-adjusts block rewards over a set number of periods.
 */
contract PlatformTokenDistributor is ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeERC20 for IPlatformToken;
    ...
    address public immutable platformTokenSplitter;
    ...
    // Tokens distributed per block for other purposes (owner + treasury)
     ...
     * @param _tokenSplitter Platform token splitter contract address (for owner and VCs)
     * @param _startBlock start block for reward program
     * @param _rewardsPerBlockForStaking array of rewards per block for staking
     * @param _rewardsPerBlockForOthers array of rewards per block for other purposes (owner + Vcs)
    ...
    constructor(
        ...
        platformTokenSplitter = _tokenSplitter;
     ...
     * @param amount amount to deposit (in Platform token)
     */
    function deposit(uint256 amount) external nonReentrant {
        ...
        // Transfer Platform tokens to this contract
    ...
    function withdraw(uint256 amount) external nonReentrant {
        ...
        // Transfer Platform tokens to the sender
    ...
    function withdrawAll() external nonReentrant {
        ...
        // Transfer Platform tokens to the sender
        platformToken.safeTransfer(msg.sender, amountToTransfer);
    ...
    function _updatePool() internal {
        ...
        if (tokenRewardForStaking > 0) {
            ...
            bool mintStatus = platformToken.mint(address(this), tokenRewardForStaking);
            if (mintStatus) {
                accTokenPerShare = accTokenPerShare + ((tokenRewardForStaking * PRECISION_FACTOR) / totalAmountStaked);
            }

            PlatformToken.mint(platformTokenSplitter, tokenRewardForOthers);
        }
